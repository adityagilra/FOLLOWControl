...
if errorLearning:                                       # PES plasticity on
    ...
    if recurrentLearning:                               # L2 recurrent learning
        ...
        if testLearned:
            ...
            PES_learning_rate_FF = 10                   # learning on Node to Neurons, hence larger rate (nengo must be hacked to allow this)
...
baseFileName = pathprefix+'inverse_Ddiff_ff_S2_d50c50'+('_ocl' if OCL else '')+'_Nexc'+str(Nexc) + \
...
if __name__ == "__main__":
    ...
    with mainModel:
        ...
        EtoE = nengo.Connection(rateEvolve, ratorOut.neurons,
                                    transform=np.zeros((Nexc,Nobs)), synapse=tau)   # synapse is tau_syn for filtering
        ...
        InEtoE = nengo.Connection(rateEvolveD, ratorOut.neurons,

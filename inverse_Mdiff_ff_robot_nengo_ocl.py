...
baseFileName = pathprefix+'inverse_Mdiff_ff_N'+str(Npre)+'_50ms'+('_ocl' if OCL else '')+'_Nexc'+str(Nexc) + \
...
if __name__ == "__main__":
    ...
    with mainModel:
        ...
        # only omega-s (angular velocities, undelayed and delayed)
        ratorInO = nengo.Ensemble( Npre, dimensions=Nobs//2, radius=reprRadiusIn,
                            neuron_type=nengo.neurons.LIF(), seed=seedR1, label='ratorIn' )
        ratorInOD = nengo.Ensemble( Npre, dimensions=Nobs//2, radius=reprRadiusIn,
                            neuron_type=nengo.neurons.LIF(), seed=seedR1, label='ratorIn' )
        nengo.Connection(rateEvolve[Nobs//2:], ratorInO, synapse=None)
        nengo.Connection(rateEvolveD[Nobs//2:], ratorInOD, synapse=None)
        ratorInT = nengo.Ensemble( Npre, dimensions=Nobs//2, radius=reprRadiusIn,
                            neuron_type=nengo.neurons.LIF(), seed=seedR1, label='ratorIn' )
        nengo.Connection(rateEvolve[:Nobs//2], ratorInT, synapse=None)
        ...
        EtoE = nengo.Connection(ratorIn.neurons, ratorOut.neurons,
        ...
        EtoET = nengo.Connection(ratorInT.neurons, ratorOut.neurons,
                                    transform=np.zeros((Nexc,Npre)), synapse=tau)
        EtoEO = nengo.Connection(ratorInO.neurons, ratorOut.neurons,
                                    transform=np.zeros((Nexc,Npre)), synapse=tau)
        EtoEOD = nengo.Connection(ratorInOD.neurons, ratorOut.neurons,
                                    transform=np.zeros((Nexc,Npre)), synapse=tau)
    ...
    with mainModel:
        if errorLearning:
            ...
            InEtoERulesDict = { 'PES' : nengo.PES(learning_rate=PES_learning_rate_FF,
                                            ...
                                            #decay_rate_x_dt=excPES_weightsDecayRate*dt,
                                            #integral_tau=excPES_integralTau) }
            EtoET.learning_rule_type = InEtoERulesDict
            EtoEO.learning_rule_type = InEtoERulesDict
            EtoEOD.learning_rule_type = InEtoERulesDict
            ...
            error_conn = nengo.Connection(\
                    errorWt,EtoET.learning_rule['PES'],synapse=dt)
            error_conn = nengo.Connection(\
                    errorWt,EtoEO.learning_rule['PES'],synapse=dt)
            error_conn = nengo.Connection(\
                    errorWt,EtoEOD.learning_rule['PES'],synapse=dt)
    ...
    if errorLearning and (continueLearning or testLearned) and isfile(weightsLoadFileName):
        ...
        with contextlib.closing(
            ...
            sim.signals[ sim.model.sig[EtoET]['weights'] ] \
                                = weights_dict['learnedTWeights']                  # can be set if weights/decoders are plastic
            sim.signals[ sim.model.sig[EtoEO]['weights'] ] \
                                = weights_dict['learnedOWeights']                  # can be set if weights/decoders are plastic
            sim.signals[ sim.model.sig[EtoEOD]['weights'] ] \
                                = weights_dict['learnedODWeights']                  # can be set if weights/decoders are plastic
    else:
    ...
    if errorLearning and not testLearned:
        with contextlib.closing(
            ...
            weights_dict['learnedTWeights'] = sim.signals[ sim.model.sig[EtoET]['weights'] ]
            weights_dict['learnedOWeights'] = sim.signals[ sim.model.sig[EtoEO]['weights'] ]
            weights_dict['learnedODWeights'] = sim.signals[ sim.model.sig[EtoEOD]['weights'] ]
